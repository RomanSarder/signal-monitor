...
import { PollQueueJob } from "../queue";
    ...
    try {
      for (const source of ["hn"]) {
        ...
        for (const keyword of monitor.keywords) {
          try {
            const sinceTimestamp = Math.floor(Date.now() / 1000) - monitor.intervalMinutes * 60;
            const hits = await hackerNewsSourceAdapter.fetchKeyword(keyword, sinceTimestamp);
              ...
              if (dedup === 1) {
                ...
                const [inserted] = await db
                  ...
                  .values({
                    ...
                    // TODO: Deduplicate results with same source id, merge keywords
